...
const OnboardingQuestionnaire = () => {
  ...
  useEffect(() => {
    ...
    const particles = [];
    const colors = ['#FDBA74', '#FB923C', '#EA580C', '#C2410C'];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-amber-50 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute -top-10 -left-10 w-40 h-40 bg-orange-200/20 rounded-full blur-xl"></div>
        <div className="absolute top-1/4 -right-10 w-60 h-60 bg-amber-200/20 rounded-full blur-xl"></div>
        <div className="absolute bottom-10 left-1/4 w-48 h-48 bg-yellow-200/20 rounded-full blur-xl"></div>
      </div>
        ...
        <div className="w-full max-w-4xl mx-auto">
          ...
          <div className={`bg-card/80 backdrop-blur-lg rounded-xl md:rounded-2xl border border-border/20 shadow-medium p-4 md:p-6 mb-6 transition-all duration-500 ${
            ...
            <div className={`grid gap-3 md:gap-4 ${
              currentQuestion.options.length <= 4 ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'
            }`}>
                  ...
                  >
                    ...
                    <div className={`mb-3 md:mb-4 p-2 md:p-3 rounded-lg md:rounded-xl w-10 h-10 md:w-12 md:h-12 flex items-center justify-center transition-all duration-300 ${
                      ...
                      <span className={`text-lg md:text-xl transition-all duration-300 ${
                        answers[currentStep] === option.id ? 'text-primary scale-110' : 'text-muted-foreground group-hover:text-foreground'
                      }`}>
                        {option.icon}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1 md:space-y-2">
                      ...
                      <p className={`text-xs md:text-sm leading-relaxed transition-colors duration-300 ${
                        answers[currentStep] === option.id ? 'text-primary' : 'text-muted-foreground'
                      }`}>
                        {option.description}
                      </p>
                    </div>
                    ...
                    <div className={`absolute inset-0 bg-gradient-to-r ${option.color} opacity-0 transition-opacity duration-300 ${
                      answers[currentStep] === option.id ? 'opacity-10' : 'group-hover:opacity-5'
                    }`}></div>
                  </button>
                </div>
              ))}
